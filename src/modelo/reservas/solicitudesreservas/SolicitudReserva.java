...
public class SolicitudReserva {
	...
	public boolean esValida(GestorLocalidad gestorLocalidad) {
		return gestorLocalidad.existeZona(iZona, jZona) && // Comprobaoms si la zona existe
				tInicial.compareTo(tFinal) < 0 && !vehiculo.getSancionado();// Y si el tiempo de reserva es válido y el
																			// vehiculo no está sancionado
	}

	public void gestionarSolicitudReserva(GestorLocalidad gestor) {
		setGestorZona(gestor.getGestorZona(iZona, jZona));// Inicializamos el GestorZona
		setHueco(gestorZona.reservarHueco(tInicial, tFinal));// Según la zona dada, intentamos reservar y guardamos el
																// hueco en la solicitud
	}
