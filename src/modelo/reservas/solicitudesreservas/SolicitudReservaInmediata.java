...
public class SolicitudReservaInmediata extends SolicitudReserva {
	...
	public boolean esValida(GestorLocalidad gestor) {
		...
		if(res && radio>0) {
			if(radio<=gestor.getRadioMaxI() && radio<=gestor.getRadioMaxJ()){ // chequeamos si el radio esta en entre los radios maximos
				res = true;
			}else if(((i=0 || i = gestor.getRadioMaxI()) && (j=0 || j=gestor.getRadioMaxJ()) && (radio =))){
	...
	public void gestionarSolicitudReserva(GestorLocalidad gestor){
		...
		double mejorPrecio; //precio de zona candidata a ser escogida
		...
		for(int a = 1; a<=radio; a++){ //bucle que recorrera todas las zonas en el orden establecido
			for(int b = 0, c = 0; b<a && c<a; b++, c++){ 
				if(i-a+b<0 || i-a+b>gestor.getRadioMaxI() || j+c<0 || j+c>gestor.getRadioMaxJ()){ //comprobamos que estamos dentro del array de arrays que seria el gestor localidad
					if(gestor.getGestorZona(i-a+b, j+c).existeHueco(tI, tF) && gestor.getGestorZona(i-a+b, j+c).getPrecio()<mejorPrecio){ //comprobamos que exista hueco y que este sea mejor que el ya definido
			...
			for(int b = 0, c = 0; b<a && c<a; b++, c++){
				if(i+b<0 || i+b>gestor.getRadioMaxI() || j+a-c<0 || j+a-c>gestor.getRadioMaxJ()){
					if(gestor.getGestorZona(i+b, j+a-c).existeHueco(tI, tF) && gestor.getGestorZona(i+b, j+a-c).getPrecio()<mejorPrecio){
			...
			for(int b = 0, c = 0; b<a && c<a; b++, c++){
				if(i+a-b<0 || i+a-b>gestor.getRadioMaxI() || j-c<0 || j-c>gestor.getRadioMaxJ()){
					if(gestor.getGestorZona(i+a-b, j-c).existeHueco(tI, tF) && gestor.getGestorZona(i+a-b, j-c).getPrecio()<mejorPrecio){
		...
		if(mejor = null){ //en el caso que no existian huecos en ninguno, escogemos el centro para ponernos en lista de espera
			mejor = gestor.getGestorZona(i, j);
		}
